...
const SubjectCreate = () => {
    ...
    const load = async () => {
      try {
        ...
        const { data: classesData } = await api.get(`/institutions/${institutionId}/classes`, {
          params: academicYearId && firstDep?._id ? { academicYearId, departmentId: firstDep._id } : undefined,
        });
        setClasses(classesData || []);
        const firstClass = classesData?.[0];
        setForm((p) => ({ ...p, classId: firstClass?._id || "" }));
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    setError("");
    try {
      await api.post(`/institutions/${institutionId}/subjects`, {
    ...
    <div className="page-container">
      <div className="page-header-row">
        <div>
          ...
          <p className="page-subtitle">Add subject under academic year, department and class.</p>
        </div>
